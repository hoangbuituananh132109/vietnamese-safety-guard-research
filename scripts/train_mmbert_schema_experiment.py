...
from guard_train.mmbert_schema import SchemaTrainingConfig, train_schema_multitask


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=Path, default=Path("configs/phase0_experiments.json"))
    parser.add_argument("--run-id", default="E7-M-SCHEMA-EV-8K")
...
def main() -> None:
    ...
    if run is None or run["model_kind"] != "mmbert_schema":
        raise ValueError(f"Unknown/non-schema run: {args.run_id}")
    ...
    config = SchemaTrainingConfig(
        ...
        enable_categories=bool(run.get("category_tasks", True)),
        category_loss_weight=float(run.get("category_loss_weight", 1.0)),
        negative_label_keep_probability=float(run.get("negative_label_keep_probability", 0.5)),
        full_category_schema_probability=float(run.get("full_category_schema_probability", 0.25)),
        save_every=int(run.get("save_every", 250)),
        log_every=int(run.get("log_every", 10)),
        seed=int(recipe["seed"]),
    )
    output_dir = args.output_root / args.run_id
    result = train_schema_multitask(
